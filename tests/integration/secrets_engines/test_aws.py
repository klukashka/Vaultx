...
class TestAws(VaultxIntegrationTestCase, TestCase):
    ...
    def test_configure_lease(self, label, lease="60s", lease_max="120s", raises=None, exception_message=""):
        if raises:
            ...
            logging.warning("configure_response: %s" % configure_response.value)
            self.assertEqual(
                first=bool(configure_response),
                second=True,
            )
    ...
    def test_read_lease(
        ...
        if raises:
            ...
            self.assertEqual(
                ...
                second=convert_python_ttl_value_to_expected_vault_response(
    ...
    def test_create_or_update_role(
        ...
        if raises:
            ...
            logging.warning("role_response: %s" % role_response.status)
...
class TestAsyncAws(AsyncVaultxIntegrationTestCase, IsolatedAsyncioTestCase):
    ...
    async def test_create_or_update_role(
        self,
        label,
        credential_type="iam_user",
        policy_document=None,
