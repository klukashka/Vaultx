...
class TestIdentity(VaultxIntegrationTestCase, TestCase):
    ...
    def tear_down_entities(self):
        try:
            ...
            logging.debug("InvalidPath raised when calling list_entities_by_id in tearDown...")
            entity_ids = []
    ...
    def tear_down_entity_aliases(self):
        try:
            list_entity_aliases_response = self.client.secrets.identity.list_entity_aliases(
                mount_point=self.TEST_MOUNT_POINT
            )
    ...
    @parameterized.expand(
        [
            ...
            param("create success with metadata", metadata={"something": "meta"}),
            param(
    ...
    @parameterized.expand(
        [
            ...
            param("read failure", create_first=False, raises=exceptions.HTTPError),
        ]
    )
    ...
    @parameterized.expand(
        [
            ...
            param("update success with metadata", metadata={"something": "meta"}),
            param(
            ...
            if isinstance(create_or_update_response, dict):
                ...
                expected_member_entity_ids = (
                    ...
                    if member_entity_ids is not None
                        ...
                        and (utils.vault_version_lt("1.9.9"))  # https://github.com/hashicorp/vault/pull/16088
                        else None
                    )
                )
                self.assertEqual(
    ...
    def test_lookup_entity(self, label, criteria, create_first=True, raises=None, exception_message=""):
        ...
        if raises:
            with self.assertRaises(raises) as cm:
                self.client.secrets.identity.lookup_entity(mount_point=self.TEST_MOUNT_POINT, **lookup_params)
            self.assertIn(
                member=exception_message,
                container=str(cm.exception),
            )
            ...
            if create_first:
                ...
                self.assertEqual(
                    first=lookup_entity_response.status,
                    second=204,
                )
    ...
    def test_lookup_group(self, label, criteria, create_first=True, raises=None, exception_message=""):  # noqa: C901
        lookup_params = {}
        if create_first:
            create_group_response = self.client.secrets.identity.create_or_update_group(
                name=self.TEST_GROUP_NAME,
        ...
        if raises:
            with self.assertRaises(raises) as cm:
                self.client.secrets.identity.lookup_group(mount_point=self.TEST_MOUNT_POINT, **lookup_params)
            self.assertIn(
                member=exception_message,
                container=str(cm.exception),
            )
            ...
            if create_first:
                ...
                self.assertEqual(
                    first=lookup_group_response.status,
                    second=204,
                )
...
class TestAsyncIdentity(AsyncVaultxIntegrationTestCase, IsolatedAsyncioTestCase):
    TEST_APPROLE_PATH = "identity-test-approle"
    TEST_APPROLE_ROLE_ID = "identity-test-role-id"
    TEST_MOUNT_POINT = "identity"
    TEST_ENTITY_NAME = "test-entity"
    ...
    async def asyncSetUp(self):
        ...
        if "%s/" % self.TEST_APPROLE_PATH not in await self.client.sys.list_auth_methods():
    ...
    async def tear_down_entity_aliases(self):
        try:
            list_entity_aliases_response = await self.client.secrets.identity.list_entity_aliases(
                mount_point=self.TEST_MOUNT_POINT
            )
            logging.debug("list_entity_aliases_response in tearDown: %s" % list_entity_aliases_response)
            alias_ids = list_entity_aliases_response["keys"]
    ...
    async def test_create_or_update_entity(
        self,
        label,
        metadata=None,
        create_first=False,
    ...
    @skipIf(utils.vault_version_lt("0.11.2"), '"by name" operations added in Vault v0.11.2')
    async def test_create_or_update_entity_by_name(
        self,
        label,
        metadata=None,
        create_first=False,
    ...
    async def test_merge_entities_conflicting(self, label, raises=None, exception_message=""):
        create_response = await self.client.secrets.identity.create_or_update_entity(
            name=self.TEST_ENTITY_NAME,
            mount_point=self.TEST_MOUNT_POINT,
        )
        ...
        logging.debug("create_response2: %s" % create_response2)
        create_response3 = await self.client.secrets.identity.create_or_update_entity(
            name="%s3" % self.TEST_ENTITY_NAME,
            mount_point=self.TEST_MOUNT_POINT,
        )
        logging.debug("create_response3: %s" % create_response3)
        ...
        if raises:
            ...
            merge_conflicting_entities_response = await self.client.secrets.identity.merge_entities(
                from_entity_ids=merge_id2,
                to_entity_id=parent_id,
                mount_point=self.TEST_MOUNT_POINT,
                conflicting_alias_ids_to_keep=[merge_id1],
    ...
    async def test_create_or_update_group(
        self,
        label,
        metadata=None,
        group_type="internal",
    ...
    async def test_update_group_by_id(
        self,
        label,
        metadata=None,
        group_type="internal",
    ...
    @skipIf(utils.vault_version_lt("0.11.2"), '"by name" operations added in Vault v0.11.2')
    async def test_create_or_update_group_by_name(
        self,
        label,
        metadata=None,
        group_type="internal",
    ...
    async def test_lookup_entity(self, label, criteria, create_first=True, raises=None, exception_message=""):
        ...
        if raises:
            with self.assertRaises(raises) as cm:
                await self.client.secrets.identity.lookup_entity(mount_point=self.TEST_MOUNT_POINT, **lookup_params)
            self.assertIn(
                member=exception_message,
                container=str(cm.exception),
            )
    ...
    async def test_lookup_group(  # noqa: C901
        ...
        if raises:
            with self.assertRaises(raises) as cm:
                await self.client.secrets.identity.lookup_group(mount_point=self.TEST_MOUNT_POINT, **lookup_params)
            self.assertIn(
                member=exception_message,
                container=str(cm.exception),
            )
    ...
    async def test_configure_tokens_backend(self, label, issuer):
        response = await self.client.secrets.identity.configure_tokens_backend(
            issuer=issuer,
        )
        logging.debug("configure_tokens_backend response: %s" % response)
        if issuer:
    ...
    async def test_read_tokens_backend_configuration(self, label, issuer):
        configure_tokens_backend_response = await self.client.secrets.identity.configure_tokens_backend(
            issuer=issuer,
        )
        logging.debug("configure_tokens_backend_response: %s" % configure_tokens_backend_response)
        response = await self.client.secrets.identity.read_tokens_backend_configuration()
        logging.debug("read_tokens_backend_configuration response: %s" % response)
        self.assertEqual(
            first=issuer,
            second=response["data"]["issuer"],
    ...
    async def test_create_named_key(self, label, name):
        response = await self.client.secrets.identity.create_named_key(
            name=name,
        )
        logging.debug("create_named_key response: %s" % response)
        self.assertEqual(
    ...
    async def test_read_named_key(self, label, name, algorithm):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=name,
            algorithm=algorithm,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        response = await self.client.secrets.identity.read_named_key(
            name=name,
        )
        logging.debug("read_named_key response: %s" % response)
        self.assertEqual(
    ...
    async def test_delete_named_key(self, label, name):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=name,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        response = await self.client.secrets.identity.delete_named_key(
            name=name,
        )
        logging.debug("delete_named_key response: %s" % response)
        self.assertEqual(
    ...
    async def test_list_named_keys(self, label, name):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=name,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        response = await self.client.secrets.identity.list_named_keys()
        logging.debug("list_named_keys response: %s" % response)
        self.assertIn(
            member=name,
            container=response["data"]["keys"],
    ...
    async def test_rotate_named_key(self, label, name, verification_ttl):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=name,
            verification_ttl=verification_ttl + 1,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        response = await self.client.secrets.identity.rotate_named_key(
            name=name,
            verification_ttl=verification_ttl,
        )
        logging.debug("rotate_named_key response: %s" % response)
        ...
        post_rotate_read_response = await self.client.secrets.identity.read_named_key(
            name=name,
        )
        logging.debug("post_rotate_read_response response: %s" % post_rotate_read_response)
        self.assertEqual(
    ...
    async def test_create_or_update_role(self, label, name, key_name):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=key_name,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        response = await self.client.secrets.identity.create_or_update_role(
            name=name,
            key=key_name,
        )
        logging.debug("create_or_update_role response: %s" % response)
    ...
    async def test_read_role(self, label, name, key_name):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=key_name,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        create_or_update_role_response = await self.client.secrets.identity.create_or_update_role(
            name=name,
            key=key_name,
        )
        logging.debug("create_or_update_role response: %s" % create_or_update_role_response)
        response = await self.client.secrets.identity.read_role(
            name=name,
        )
        logging.debug("read_role response: %s" % response)
        self.assertEqual(
    ...
    async def test_delete_role(self, label, name, key_name):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=key_name,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        ...
        logging.debug("create_or_update_role response: %s" % create_or_update_role_response)
        response = await self.client.secrets.identity.delete_role(
            name=name,
        )
        logging.debug("delete_role response: %s" % response)
        self.assertEqual(
    ...
    async def test_list_roles(self, label, name, key_name):
        create_named_key_response = await self.client.secrets.identity.create_named_key(
            name=key_name,
        )
        logging.debug("create_named_key response: %s" % create_named_key_response)
        ...
        logging.debug("create_or_update_role response: %s" % create_or_update_role_response)
        response = await self.client.secrets.identity.list_roles()
        logging.debug("list_roles response: %s" % response)
        self.assertIn(
            member=name,
            container=response["data"]["keys"],
    ...
    # TODO: Figure out why this test gets 403: Forbidden on DELETE
    # @parameterized.expand(
    #     [
    #         param(
    #             "success",
    ...
    #     ]
    # )
    # async def test_generate_signed_id_token(self, label, role_name, key_name):
    #     create_or_update_role_response = (
    #         await self.client.secrets.identity.create_or_update_role(
    #             name=role_name,
    #             key=key_name,
    #         )
    #     )
    ...
    #     )
    #     read_role_response = await self.client.secrets.identity.read_role(
    #         name=role_name,
    #     )
    #     logging.debug("read_role response: %s" % read_role_response)
    #     token_client_id = read_role_response["data"]["client_id"]
    ...
    #     logging.debug("create_named_key response: %s" % create_named_key_response)
    #     # Log in using a dummy approle role so our client token has an associated identity
    #     await self.login_using_admin_approle_role(
    #         role_id=self.TEST_APPROLE_ROLE_ID,
    #         path=self.TEST_APPROLE_PATH,
    #     )
    #     response = await self.client.secrets.identity.generate_signed_id_token(
    #         name=role_name,
    #     )
    #     logging.debug("generate_signed_id_token response: %s" % response)
    #     self.assertIn(
    ...
    #     ]
    # )
    # async def test_introspect_signed_id_token(self, label, role_name, key_name):
    #     create_or_update_role_response = (
    #         await self.client.secrets.identity.create_or_update_role(
    #             name=role_name,
    #             key=key_name,
    ...
    #         path=self.TEST_APPROLE_PATH,
    #     )
    #     generate_signed_id_token_response = await self.client.secrets.identity.generate_signed_id_token(
    #             name=role_name,
    #         )
    #     logging.debug(
    #         "generate_signed_id_token response: %s" % generate_signed_id_token_response
    #     )
    #     response = await self.client.secrets.identity.introspect_signed_id_token(
    #         token=generate_signed_id_token_response["data"]["token"],
    #         client_id=token_client_id,
    #     )
    #     logging.debug("introspect_signed_id_token response: %s" % response)
    ...
    async def test_read_well_known_configurations(self, label, issuer):
        await self.client.secrets.identity.configure_tokens_backend(
            issuer=issuer,
        )
        response = await self.client.secrets.identity.read_well_known_configurations()
        logging.debug("read_well_known_configurations response: %s" % response)
        self.assertIn(
            member=issuer,
            container=response["issuer"],
    ...
    async def test_read_active_public_keys(self, label, issuer):
        await self.client.secrets.identity.configure_tokens_backend(
            issuer=issuer,
        )
        response = await self.client.secrets.identity.read_active_public_keys()
        logging.debug("read_active_public_keys response: %s" % response)
        self.assertIn(
            member="keys",
            container=response,
