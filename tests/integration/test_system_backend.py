...
class TestSystemBackend(VaultxIntegrationTestCase, TestCase):
    ...
    def test_audit_backend_manipulation(self):
        ...
        self.assertNotIn("tmpfile/", self.client.sys.list_enabled_audit_devices()["data"])
    ...
    def test_policy_manipulation(self):
        ...
        a = self.client.get_policy("test", parse=True)
        logging.warning(f"here is get policy type {type(a)}: {a}")
        logging.warning(f"here is parsed_policy type:{type(parsed_policy)}: {parsed_policy}")
        self.assertEqual(parsed_policy, a)
    ...
    def test_read_lease(self):
        ...
        pki_issue_response = self.client.write(
            ...
            data={"common_name": "test.vaultx.com"},
        )
    ...
    def test_audit_hash(
        ...
        if enable_first:
            options = {"path": "/tmp/vault.audit.log"}
            self.client.sys.enable_audit_device("file", options=options, path=audit_backend_path)

        if raises:
            with self.assertRaises(raises) as cm:
                self.client.sys.calculate_hash(path=audit_backend_path, input_to_hash=test_input)
    ...
    def test_get_secret_backend_tuning(self):
        secret_backend_tuning = self.client.sys.read_mount_configuration(self.TEST_KVV1_MOUNT_POINT)
        self.assertIn(
...
class TestAsyncSystemBackend(AsyncVaultxIntegrationTestCase, IsolatedAsyncioTestCase):
    TEST_KVV1_MOUNT_POINT = "kvv1_mount"

    async def asyncSetUp(self):
        await super().asyncSetUp()
        if "%s/" % self.TEST_KVV1_MOUNT_POINT not in (await self.client.sys.list_mounted_secrets_engines())["data"]:
    ...
    async def asyncTearDown(self):
        self.client.token = self.manager.root_token
        await self.client.sys.disable_secrets_engine(path=self.TEST_KVV1_MOUNT_POINT)
    ...
    async def test_wrap_write(self):
        if "approle/" not in await self.client.sys.list_auth_methods():
            await self.client.sys.enable_auth_method("approle")
        ...
        self.assertIn("token", result["wrap_info"])
        await self.client.sys.unwrap(result["wrap_info"]["token"])
        await self.client.sys.disable_auth_method("approle")

    async def test_auth_backend_manipulation(self):
        self.assertNotIn("github/", (await self.client.sys.list_auth_methods())["data"])

        await self.client.sys.enable_auth_method("github")
        self.assertIn("github/", (await self.client.sys.list_auth_methods())["data"])

        self.client.token = self.manager.root_token
        await self.client.sys.disable_auth_method("github")
        self.assertNotIn("github/", (await self.client.sys.list_auth_methods())["data"])
    ...
    async def test_secret_backend_manipulation(self):
        ...
        await self.client.sys.enable_secrets_engine("kv", path="test")
        self.assertIn("test/", (await self.client.sys.list_mounted_secrets_engines())["data"])
    ...
    async def test_audit_backend_manipulation(self):
        ...
        await self.client.sys.enable_audit_device("file", options=options, path="tmpfile")
        self.assertIn("tmpfile/", (await self.client.sys.list_enabled_audit_devices())["data"])

        self.client.token = self.manager.root_token
        await self.client.sys.disable_audit_device("tmpfile")
        self.assertNotIn("tmpfile/", (await self.client.sys.list_enabled_audit_devices())["data"])

    async def test_policy_manipulation(self):
        self.assertIn("root", (await self.client.sys.list_policies())["keys"])
        with self.assertRaises(exceptions.HTTPError):
            await self.client.sys.read_policy("test")

        policy, parsed_policy = await self.prep_policy("test")
        self.assertIn("test", (await self.client.sys.list_policies())["keys"])
        test_policy_read_resp = await self.client.sys.read_policy("test")
        logging.debug("test_policy_read_resp: %s" % test_policy_read_resp)
        self.assertEqual(policy, test_policy_read_resp["rules"])
        a = await self.client.get_policy("test", parse=True)
        logging.warning(f"here is get policy type {type(a)}: {a}")
        logging.warning(f"here is parsed_policy type:{type(parsed_policy)}: {parsed_policy}")
        self.assertEqual(parsed_policy, a)

        await self.client.sys.delete_policy("test")
        self.assertNotIn("test", (await self.client.sys.list_policies())["keys"])
    ...
    async def test_json_policy_manipulation(self):
        ...
        """
        await self.client.sys.create_or_update_policy("test", policy)
        self.assertIn("test", (await self.client.sys.list_policies())["keys"])
    ...
    async def test_cubbyhole_auth(self):
        ...
        resp = await self.client.auth.token.create(ttl="6h", wrap_ttl="1h")
        self.assertEqual(resp["wrap_info"]["ttl"], 3600)
    ...
    async def test_wrapped_token_success(self):
        wrap = await self.client.auth.token.create(wrap_ttl="1m")

        # Unwrap token
        result = await self.client.sys.unwrap(wrap["wrap_info"]["token"])
        self.assertTrue(result["auth"]["client_token"])
    ...
    async def test_wrapped_token_intercept(self):
        ...
        # Intercept wrapped token
        await self.client.sys.unwrap(wrap["wrap_info"]["token"])
    ...
    async def test_wrapped_token_revoke(self):
        ...
        # Revoke token before it's unwrapped
        await self.client.auth.token.revoke_accessor(wrap["wrap_info"]["wrapped_accessor"])
    ...
    async def test_wrapped_client_token_success(self):
        ...
        # Unwrap token
        result = await self.client.sys.unwrap()
        self.assertTrue(result["auth"]["client_token"])
    ...
    async def test_start_generate_root_with_completion(self):
        ...
        logging.debug("new_root_token: %s" % new_root_token)
        token_lookup_resp = await self.client.auth.token.lookup(token=new_root_token)
        logging.debug("token_lookup_resp: %s" % token_lookup_resp)
    ...
    async def test_tune_auth_backend(self):
        ...
        test_description = "this is a test auth backend"
        test_max_lease_ttl = 12345678
        if f"{test_mount_point}/" in await self.client.sys.list_auth_methods():
            await self.client.sys.disable_auth_method(test_mount_point)
        await self.client.sys.enable_auth_method(method_type="approle", path=test_mount_point)
        ...
        response = await self.client.sys.read_auth_method_tuning(path=test_mount_point)
    ...
    async def test_read_lease(self):
        # Set up a test pki backend and issue a cert against some role so we.
        utils.configure_pki(client=self.manager.client)
        ...
        # Read the lease of our test cert that was just issued.
        read_lease_response = await self.client.sys.read_lease(pki_issue_response["lease_id"])
        ...
        # Reset integration test state.
        utils.disable_pki(client=self.manager.client)
    ...
    async def test_audit_hash(
        ...
        if enable_first:
            options = {"path": "/tmp/vault.audit.log"}
            await self.client.sys.enable_audit_device("file", options=options, path=audit_backend_path)

        if raises:
            with self.assertRaises(raises) as cm:
                await self.client.sys.calculate_hash(path=audit_backend_path, input_to_hash=test_input)
            if exception_message is not None:
                self.assertIn(
                    member=exception_message,
                    container=str(cm.exception),
    ...
    async def test_get_secret_backend_tuning(self):
        secret_backend_tuning = await self.client.sys.read_mount_configuration(self.TEST_KVV1_MOUNT_POINT)
        self.assertIn(
            member="default_lease_ttl",
            container=secret_backend_tuning["data"],
        )
