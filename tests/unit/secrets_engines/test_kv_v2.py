...
class TestKvV2(unittest.TestCase):
    ...
    def test_patch_updates_secret_and_returns_response(self):
        mock_read_response = VaultxResponse(
            Response(200, json={"data": {"data": {"key": "old-value"}, "metadata": {"version": 1}}})
        )
        mock_write_response = VaultxResponse(Response(200, json={"data": {"version": 2}}))
        ...
        self.assertEqual(result.value, {"data": {"version": 2}})
        self.mock_adapter.post.assert_called_once_with(
            url="/v1/secret/data/my-secret",
