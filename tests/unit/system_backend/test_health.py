...
from vaultx.api.async_system_backend.health import Health as AsyncHealth
...
class TestAsyncHealth(IsolatedAsyncioTestCase):
    def setUp(self):
        self.mock_adapter = mock.AsyncMock()
        self.health = AsyncHealth(self.mock_adapter)
    ...
    async def test_read_health_status_get(self):
        self.mock_adapter.get.return_value = {"initialized": True}
        result = await self.health.read_health_status(method="GET")
        self.assertEqual(result, {"initialized": True})
        self.mock_adapter.get.assert_called_once_with(
            url="/v1/sys/health",
            params={},
    ...
    async def test_read_health_status_get_with_params(self):
        self.mock_adapter.get.return_value = {"initialized": True}
        result = await self.health.read_health_status(
            standby_ok=True,
            active_code=200,
            standby_code=200,
            dr_secondary_code=200,
    ...
    async def test_read_health_status_invalid_method(self):
        with self.assertRaises(VaultxError) as context:
            await self.health.read_health_status(method="POST")
        self.assertEqual(
            str(context.exception),
            '"method" parameter provided invalid value; HEAD or GET allowed, "POST" provided',
        )
